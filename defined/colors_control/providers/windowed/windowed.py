from colors_control.objects import ColorProvider, ColoredDevice
...
class WindowedColorProvider(ColorProvider):
    def __init__(self) -> None:
        super().__init__("Windowed Based Color", '')
    ...
    def capture_pattern(self) -> ComplexKeyboardColorPattern:
        ...
        with XLIB_LOCK:
            ...
            if active_window is not None:
                ...
                position = active_window.position

                image = image.crop((position[0], position[1], position[0] + size[0], position[1] + size[1]))
