...
from .devices import ComplexColorProviderSavedDeviceDirectory
from defined.colors_control.devices.keyboard.patterns import KeyboardPattern

class ComplexColorProviderSaves(_configuration.SettingsDirectory):
    def __init__(self, path: str) -> None:
        super().__init__(path)

    def get_device_save(self, device_name: str, default_pattern: KeyboardPattern) -> ComplexColorProviderSavedDeviceDirectory:
        return ComplexColorProviderSavedDeviceDirectory(self._create_sub_element_path(device_name), default_pattern)
