...
class ColorPattern:
    ...
    @property
    def colors(self) -> _T.Sequence[Color]:
    ...
    def ensure_pattern_length_compatibility(self, other: _T.Self) -> tuple[_T.Sequence[Color], _T.Sequence[Color]]:
    ...
    def interpolate_rgb_to(self, other: _T.Self, t: float) -> _T.Self:
        cls = type(self)

        colors1, colors2 = self.ensure_pattern_length_compatibility(other)
        
        result = [a.interpolate_rgb(b or Color(0), t) for a, b in zip(colors1, colors2)]
        return cls(*result)

    def interpolate_hsv_to(self, other: _T.Self, t: float) -> _T.Self:
        cls = type(self)
